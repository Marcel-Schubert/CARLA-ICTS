...
class CI3PP_CVAE_ATT(nn.Module):
    def __init__(self, latent_dim=24, predict_frames=20):
        super(CI3PP_CVAE_ATT, self).__init__()
        ...
        self.h_traj = nn.GRU(128, 64)
        self.h_ci = nn.GRU(128, 64)

        self.fc_query_traj = nn.Linear(64, 64)
        self.fc_key_traj = nn.Linear(64, 64)
        self.fc_value_traj = nn.Linear(64, 64)

        self.fc_query_ci = nn.Linear(64, 64)
        self.fc_key_ci = nn.Linear(64, 64)
        self.fc_value_ci = nn.Linear(64, 64)
        ...
        self.mu = nn.Linear(256 + 128, self.latent_dim)
        self.var = nn.Linear(256 +128, self.latent_dim)

        self.decoder_linear = LinearTanh(self.latent_dim + 128, 128)
    ...
    def forward(self, x):
        ...
        x1 = torch.cat((traj_attention, cf_attention), dim=-1).sum(dim=0).unsqueeze(dim=0)
