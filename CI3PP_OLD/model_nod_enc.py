...
class CI3PP_NO_ENC(nn.Module):

    def __init__(self, n_observed_frames, n_predict_frames):
        super(CI3PP_NO_ENC, self).__init__()
        self.n_pred = n_predict_frames
        self.n_obs = n_observed_frames

        self.tdfc_traj = TimeDistributed(LinearReLu(2, 128), batch_first=False)
        self.tdfc_cf = TimeDistributed(LinearReLu(2, 128), batch_first=False)

        self.encoder_traj = nn.GRU(input_size=128, hidden_size=128)
        self.encoder_cf = nn.GRU(input_size=128, hidden_size=128)

        # generate query, keys and values
        self.fc_query_traj = nn.Linear(2, 128)
        self.fc_key_traj = nn.Linear(2, 128)
        self.fc_value_traj = nn.Linear(2, 128)

        self.fc_query_cf = nn.Linear(2, 128)
        self.fc_key_cf = nn.Linear(2, 128)
        self.fc_value_cf = nn.Linear(2, 128)
    ...
    def forward(self, x_traj, x_cf):
        ...
        query_traj = self.fc_query_traj(x_traj)
        key_traj = self.fc_key_traj(x_traj)
        value_traj = self.fc_value_traj(x_traj)

        query_cf = self.fc_query_cf(x_cf)
        key_cf = self.fc_key_cf(x_cf)
        value_cf = self.fc_value_cf(x_cf)

        traj_attention, _ = self.mha_traj(query_cf, key_traj, value_traj)
        cf_attention, _ = self.mha_cf(query_traj, key_cf, value_cf)

        stacked = torch.cat((traj_attention, cf_attention), dim=-1)[0]
