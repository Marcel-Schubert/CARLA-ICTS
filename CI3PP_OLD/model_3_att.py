...
class CI3PP(nn.Module):
    def __init__(self, n_observed_frames, n_predict_frames):
        ...
        self.pos_enc_com = PositionalEncoding(256)
        ...
        # 3rd attention
        self.fc_query_3rd = nn.Linear(256, 256)
        self.fc_key_3rd = nn.Linear(256, 256)
        self.fc_value_3rd = nn.Linear(256, 256)
        ...
        # common attention
        self.mha_com = nn.MultiheadAttention(embed_dim=256, num_heads=8)
    ...
    def forward(self, x_traj, x_cf):
        ...
        stacked = self.pos_enc_com(stacked)


        q_3rd = self.fc_query_3rd(stacked)
        k_3rd = self.fc_key_3rd(stacked)
        v_3rd = self.fc_value_3rd(stacked)

        comm_attention, _ = self.mha_com(q_3rd, k_3rd, v_3rd)

        _, comm_enc = self.enc_com(comm_attention)

        decoded_lin = self.decoder_linear(comm_enc).repeat(self.n_pred, 1, 1)
