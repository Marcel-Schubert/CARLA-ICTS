...
class CI3PP_CVAE_ATT_NOX(nn.Module):
    def __init__(self, latent_dim=24, predict_frames=20, n_heads=4, embed_dim=64):
        super(CI3PP_CVAE_ATT_NOX, self).__init__()
        ...
        self.h_traj = nn.GRU(128, embed_dim)
        self.h_ci = nn.GRU(128, embed_dim)
        ...
        self.mu = nn.Linear(256 + embed_dim*2, self.latent_dim)
        self.var = nn.Linear(256 + embed_dim*2, self.latent_dim)

        self.decoder_linear = LinearTanh(self.latent_dim + embed_dim*2, 128)
    ...
    def forward(self, x):
        ...
        traj_attention, _ = self.mha_traj(enc_traj, enc_traj_seq, enc_traj_seq)
        cf_attention, _ = self.mha_cf(enc_cf, enc_cf_seq, enc_cf_seq)


        x1 = torch.cat((traj_attention, cf_attention), dim=-1)
